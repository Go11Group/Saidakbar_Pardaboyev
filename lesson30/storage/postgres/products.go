...
func (p *Products) DeleteProduct(product *model.Product) error {
	tr, err := p.DB.Begin()
	if err != nil {
		return err
	}
	...
	query := `update products set id=$1, name=$2, description=$3, price=$4,
	stock_quantity=$5, createdat=$6, updatedat=$7, deletedat=$8 where id=$9`
	_, err = tr.Exec(query, product.Id, product.Name, product.Description, 
		product.Price, product.Stock_quantity, product.CreatedAt,
		product.UpdatedAt, time.Now(), product.Id)
	if err != nil {
		return err
	}
	return nil
