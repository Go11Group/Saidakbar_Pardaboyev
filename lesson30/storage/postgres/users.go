...
func (u *Users) DeleteUser(user model.User) error {
	tr, err := u.DB.Begin()
	if err != nil {
		return err
	}
	...
	query := `update users set id=$1, username=$2, email=$3, password=$4,
	createdat=$5, updatedat=$6, deletedat=$7 where id=$8`
	_, err = tr.Exec(query, user.Id, user.Username, user.Email, user.Password,
		user.CreatedAt, user.UpdatedAt, time.Now(), user.Id)
	if err != nil {
		return err
	}
	return nil
